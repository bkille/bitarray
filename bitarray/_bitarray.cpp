...
typedef unsigned long long big_word;
...
struct bitarrayobject {
    PyObject_VAR_HEAD
    std::vector<big_word, PyAlloc<big_word> >* ob_item;
...
#define WORDS(bits)  (((bits) == 0) ? 0 : (((bits) - 1) / bit::binary_digits<big_word>::value + 1))
#define BITS(bytes)  ((idx_t) (bytes) << 3)


/* ------------ low level access to bits in bitarrayobject ------------- */
static void
print_bitarray(bitarrayobject* self) {
    for (int i = 0; i < self->nbits; ++i)
        std::cout << (self->bits[i]  ? 1 : 0);
    std::cout << std::endl;
}
...
resize(bitarrayobject* self, idx_t nbits)
{
    ...
    self->ob_item->resize(BYTES(nbits));
    if (nbits < old_nbits) {
        self->ob_item->shrink_to_fit();
    }
    self->nbits = nbits;
    Py_SIZE(self) = self->ob_item->size();
...
newbitarrayobject(PyTypeObject* type, idx_t nbits, int endian)
{
    ...
    nwords = (Py_ssize_t) WORDS(nbits);
    Py_SIZE(obj) = nwords;
    obj->nbits = nbits;
    obj->endian = endian;
    obj->ob_item = new std::vector<big_word, PyAlloc<big_word> >(nwords, 0);
    if (obj->ob_item == NULL) {
        PyObject_Del(obj);
        PyErr_NoMemory();
        return NULL;
...
setunused(bitarrayobject* self)
{
    ...
    constexpr size_t digits = bit::binary_digits<big_word>::value;
    idx_t i;
    int res = 0;
...
{
    if ((bit::binary_digits<big_word>::value * self->ob_item->size()) == self->nbits) {
        self->ob_item->push_back(static_cast<big_word>(0));
        Py_SIZE(self) = self->ob_item->size();
        self->bits = bit::bit_iterator(self->ob_item->begin());
    }
    self->nbits += 1;
...
extend_rawbytes(bitarrayobject* self, PyObject *bytes)
{
    ...
    std::copy(
            str, 
            str + strlen, 
            reinterpret_cast<char*>(&*self->ob_item->begin()) + Py_SIZE(self) - strlen);
    return 0;
}
...
bitarray_copy(bitarrayobject* self)
{
    ...
    *((bitarrayobject*) res)->ob_item = *self->ob_item;
    return res;
}
...
bitarray_buffer_info(bitarrayobject *self)
{
    ...
    ptr = PyLong_FromVoidPtr(&(*self->ob_item)[0]),
    res = Py_BuildValue("OLsiL",
                        ptr,
                        (idx_t) Py_SIZE(self),
                        ENDIAN_STR(self),
                        (int) (BITS(Py_SIZE(self)) - self->nbits),
                        (idx_t) self->ob_item->capacity());
    Py_DECREF(ptr);
    return res;
}
...
bitarray_reduce(bitarrayobject *self)
{
    ...
    memcpy(str + 1, &*self->ob_item->begin(), Py_SIZE(self));
...
{
    for (auto it = self->ob_item->begin(); it != self->ob_item->end(); ++it) {
        *it = bit::_bitswap(*it);
    }
    Py_RETURN_NONE;
}
...
bitarray_sort(bitarrayobject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (reverse) {
        ...
        std::sort(self->bits, self->bits + self->nbits);
    }
    Py_RETURN_NONE;
}
...
bitarray_fromfile(bitarrayobject *self, PyObject *args)
{
    ...
    if (result != NULL) {
        ...
        memcpy(&(*self->ob_item)[0] + (Py_SIZE(self) - nread),
...
bitarray_fromfile(bitarrayobject *self, PyObject *args)
{
    ...
    nread = fread(&(*self->ob_item)[0] + (Py_SIZE(self) - nbytes), 1, nbytes, fp);
...
bitarray_tofile(bitarrayobject *self, PyObject *f)
{
    ...
    value = PyBytes_FromStringAndSize(
            reinterpret_cast<char*>(&(*self->ob_item->begin())), 
            Py_SIZE(self));
    if (value == NULL) {
        Py_DECREF(writer);
        return NULL;
...
bitarray_tofile(bitarrayobject *self, PyObject *f)
{
    ...
    setunused(self);
    if (fwrite(&(*self->ob_item)[0], 1, Py_SIZE(self), fp) !=
        (size_t) Py_SIZE(self))
    {
...
bitarray_tolist(bitarrayobject *self)
{
    ...
    for (i = 0; i < self->nbits; i++) {
        // TODO use Py_True and Py_False
        if (PyList_SetItem(list, (Py_ssize_t) i, 
                    PyBool_FromLong(self->bits[i] ? 1 : 0)) < 0) {
            return NULL;
        }
    }
    return list;
...
bitarray_pop(bitarrayobject *self, PyObject *args)
{
    ...
    bool vi = static_cast<bool>(self->bits[i]);
...
bitarray_getitem(bitarrayobject *self, PyObject *a)
{
    ...
    if (IS_INDEX(a)) {
        ...
        // TODO use Py_True
        return PyBool_FromLong(self->bits[i] ? 1 : 0);
    }
    if (PySlice_Check(a)) {
        ...
        for (i = 0, j = start; i < slicelength; i++, j += step)
            ((bitarrayobject *) res)->bits[i] = self->bits[j];
...
#define np  ((bitarrayobject *) initial)
        ...
        if (a == NULL)
            return NULL;
        memcpy(&(*((bitarrayobject *) a)->ob_item)[0], &(*np->ob_item)[0], Py_SIZE(np));
#undef np
        return a;
    }
    ...
    if (PyBytes_Check(initial)) {
        ...
        if (0 <= str[0] && str[0] < 8) {
            ...
            std::move(
                str + 1,
                str + strlen, 
                reinterpret_cast<char*>(&*((bitarrayobject*) a)->ob_item->begin()));
            //TODO Not sure why this is necessary...
            //bitarray_bytereverse((bitarrayobject*) a);
